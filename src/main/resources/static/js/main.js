...
// --- 3. Хайлтын функц (Global scope) ---
function setupSearch() {
    const searchInput = document.getElementById('searchInput') || document.querySelector('.custom-search-input');
    if (searchInput) {
        searchInput.onkeyup = function () {
            console.log("Хайж байна: " + this.value);
        };
    } else {
        console.log("Хайлтын талбар олдсонгүй!");
    }
}
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // Register товч холбох
    if (createAccBtn) {
        createAccBtn.onclick = function (e) {
            e.preventDefault();
            openRegisterModal();
        };
    }
...
// main.js дээрх resize функцээ ингэж шинэчилээрэй
function adjustModalSize(iframeSelector, contentSelector) {
    const iframe = document.querySelector(iframeSelector);
    const modalContent = document.querySelector(contentSelector);
    
    if (iframe && iframe.contentWindow && iframe.contentWindow.document.body) {
        // Доторх хуудасны жинхэнэ өндрийг авах
        const newHeight = iframe.contentWindow.document.body.scrollHeight;
        
        // Iframe болон гаднах хайрцагны өндрийг ижил болгох
...
// 0.5 секунд тутамд шалгаж өндрийг тааруулна (Register-ээс Login руу шилжихэд хэрэгтэй)
setInterval(() => {
    adjustModalSize('.login-iframe', '.login-modal-content');
    adjustModalSize('.register-iframe', '.register-modal-content'); // Хэрэв класс нь өөр бол
}, 500);